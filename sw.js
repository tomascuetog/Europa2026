...
self.addEventListener('fetch',e=>{
  ...
  var pn = new URL(e.request.url).pathname;
  // HTML: network-first — siempre mostrar la versión más reciente; caché como fallback offline
  if(/\.html$/i.test(pn) || pn==='/' || pn===''){
    e.respondWith(fetch(e.request).then(res=>{if(res.status===200){var c=res.clone();caches.open(CACHE).then(cache=>cache.put(e.request,c));}return res;}).catch(()=>caches.match(e.request)));
    return;
  }
